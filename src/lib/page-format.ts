...
export type PageSettings = {
  size: PaperSize;
  marginIn: number;
  /** Extra clearance between body text and footer (paper pixels at 96dpi). */
...
/** Fixed 96dpi pixel size — layout always uses these, then scales to fit the viewport. */
const PAPER_PIXELS: Record<PaperSize, { width: number; height: number }> = {
  letter: { width: 816, height: 1056 },
  a4: { width: 794, height: 1123 },
};

/** Visual gap between stacked pages (in paper-coordinate pixels). */
export const PAGE_GAP_PX = 24;

/** Padding between header/footer chrome and the body text area. */
export const BODY_TEXT_GAP_PX = 20;

/** Default extra clearance above the footer so text does not overlap. */
export const DEFAULT_BODY_BOTTOM_INSET_PX = 24;
...
export function getPaperPixels(size: PaperSize) {
  return PAPER_PIXELS[size];
}
